...
SPEC_FILE="examples/launch-specs/cloudhypervisor-execute.json"
...
if command -v python3 >/dev/null 2>&1; then
    ...
    curl -s -X POST -H "Content-Type: application/json" -d @"$TMP_SPEC" "${API_BASE}/api/v1alpha1/launch/execute" | python3 -m json.tool || true
else
    sed "s/PUT_NODE_ID_HERE/${NODE_ID}/g" "$SPEC_FILE" > "$TMP_SPEC"
    curl -s -X POST -H "Content-Type: application/json" -d @"$TMP_SPEC" "${API_BASE}/api/v1alpha1/launch/execute"
    echo ""
fi